    ...
    def a_star(EstacaoAtual, EstacaoDestino, HorarioAtual, LinhaAtual):
        ...
        for EstacaoAdj in DistanciaAdj[EstacaoAtual]:
            if EstacaoAdj[0] not in Caminho:
                ...
                # print('Trajeto {} - {} // Entrada {}:{} - Chegada {}:{} // CaminhoTeste {} - MenorCaminho {}'.format(EstacaoAtual, EstacaoAdj[0], str(int(HorarioAtual//60)+4).zfill(2), str(round(HorarioAtual%60)).zfill(2), str(int(HorarioAtual + HorarioTeste//60)+4).zfill(2), str(round(HorarioAtual + HorarioTeste%60)).zfill(2), CaminhoTeste, MenorCaminho))
        ...
        if ProxEstacao == EstacaoDestino:
            Caminho.append(ProxEstacao)
